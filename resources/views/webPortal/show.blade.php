...
@section('content')
    <div class="row">
        <div class="col-lg-12 margin-tb">
            <div class="pull-left">
                <h2>{{$clientDetails['client_master_name']}} </h2>
            </div>
            <div class="pull-right">
               
               @can('client-list')
                <a class="btn btn-success" href="{{ route('webportal.create',['id'=>$web_portal_client_id]) }}"> Create</a>
                <a class="btn btn-success" href="{{ route('webportal.download',$web_portal_client_id) }}"> Download</a>
                
                @endcan
                <a class="btn btn-success" href="{{ route('webportal.index') }}"> Back</a>
                @if ($message = Session::get('success'))
                     <div class="alert alert-success">
                         <p>{{ $message }}</p>
                     </div>
                 @endif
                 <?php $qestion = config('constants.question');?>
                <table class="table table-bordered data-table display nowrap">
                    <thead>
                        <tr>
                            ...
                            <th>Password</th>
                            <th>Weblink</th>
                            <th>Registered Email / Assigned to</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{$key+1}}</td>
                            <td>{{$value->web_portal_insurance_pms}}</td>
                            <td>{{$value->web_portal_username}}</td>
                            <td>{{$value->web_portal_password}}</td>
                            <td>{{$value->web_portal_weblink}}</td>
                            <td>{{$value->web_portal_register_email}}</td>
                            <td>{{$value->web_portal_registered_phone}}</td>
                            <td>
                                {{$value->web_portal_security_q}}
                                <!--{{isset($qestion[$value->web_portal_security_q])?$qestion[$value->web_portal_security_q]:''}}-->
                            </td>
                            @can('client-list')
                            <!--<td>{{$value->web_portal_security_a}}</td>-->
                            <td><a href="{{ route('webportal.edit',$value->user_reassign_master_id) }}">Edit</a></td>
                            @endcan
                        </tr>
                        @endforeach
                        @if(count($list)==0)
