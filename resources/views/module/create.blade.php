...
{!! Form::open(array('route' => 'module.store','method'=>'POST')) !!}
            ...
            <div class="row">
                @foreach($fields as $key => $value)
                    <div class="col-xs-3 col-sm-3 col-md-3">
                        <strong>{{trans('module.'.$value)}}</strong>                      
                        <div class="input-group">
                            @php
                                $val = isset($billingmaster[$value])?$billingmaster[$value]:null;
                            @endphp
                            <input type='text' class='form-control' id='{{$value}}' name='{{$value}}' value="{{$val}}" placeholder="Enter module name">
                        </div>
                    </div>
                <div>
                    <a  class="add-date-row" onclick="removeWoc(this)">
                        <input type="hidden" name="currentindex" id="currentindex_" value="">                                
