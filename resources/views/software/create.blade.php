...
{!! Form::open(array('route' => 'software.store','method'=>'POST')) !!}
            ...
            <div class="row">
                @foreach($fields as $key => $value)
                    <div class="col-xs-3 col-sm-3 col-md-3">
                        <strong>{{trans('software.'.$value)}}</strong>                      
                        <div class="input-group">
                            @php
                                $val = isset($billingmaster[$value])?$billingmaster[$value]:null;
                            @endphp
                            <input type='text' class='form-control' id='{{$value}}' name='{{$value}}' value="{{$val}}" placeholder="Enter software name">
                        </div>
                    </div>
                @endforeach
            </div>
